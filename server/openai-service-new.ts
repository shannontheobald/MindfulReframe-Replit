...
import { getRules, applySecurityFilters, isAllowedAIUseCase, sanitizeUserInput, isCrisisText, getCrisisResponse, shouldBlockPromptInjection, getAssistantTonePrompt, getModelForTask, getMaxTokensForOperation } from '../shared/rules';
...
export async function analyzeJournalEntry(
  ...
  if (!isAllowedAIUseCase('analyzeJournalEntry')) {
    throw new Error("Journal analysis is not currently allowed");
  }
  ...
  const filteredEntry = applySecurityFilters(journalEntry);
  
  // Check for crisis indicators
  if (isCrisisText(filteredEntry)) {
    return {
...
` : '';
  ...
  const prompt = `${getAssistantTonePrompt()}
